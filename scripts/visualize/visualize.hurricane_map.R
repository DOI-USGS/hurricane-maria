...
visualize_hurricane_map <- function(viz=as.viz('hurricane-map-mobile'), mode, ...){
  ...
  xml_add_child(g.sparkle.blck, 'text', x=as.character(side.panel/2), '(normalized stage)', dy='4em', 'text-anchor'='middle', class='svg-text smallprint-text legend-text')
...
add_legend <- function(parent.ele, colors, break.step, mode){
  ...
  g.main_gage_text <- xml_add_child(parent.ele, 'text', "USGS Stream Gages (< 25% of Puerto Rico gages)", class='svg-text legend-text', dy="-1em",
